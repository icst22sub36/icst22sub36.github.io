import { internalToString, getGlobalObject, toInt } from "./utils";
import { RegExp } from './RegExpLib';
import { dynamicRegExpCreation, getNative } from './DynRegExpLib'
import { isFailure, makeFailure } from "./Failure";
import { endIndex, captures, state } from "./State";
...
function match(re) {
  if (this === undefined) { throw new TypeError ( "String.prototype.match cannot be used as a constructor") }
...
function consumeOneOrTwoDigits(i, str) { 
  ...
  var xi = parseInt(x); 
  var yi = parseInt(y); 
  if (isNaN(xi)) { return null }
  if (isNaN(yi)) { return xi }
  return xi*10 + yi; 
}
...
function resolveDollar (i, rpl_str, match, index, caps, og_str) { 
  ...
  switch (rpl_str[i+1]) {
    ...
    case "&" : return ret(i+2, match); 

    case "'" : return ret(i+2, og_str.substring(index + match.length)); 

    case "`" : return ret(i+2, og_str.substring(0, index)); 
...
function split(separator, limit) {
  ...
  var a = new Array();
  var a_len = 0;
  var lim = limit == undefined ? Math.pow(2, 32) - 1 : toInt(limit);
  ...
  //console.log("split 3: "+ "p, q: " + p)
  var q = p;

  while (q !== s_len) {
    var z = splitMatch(s, q, r);

    //console.log("main loop with q = " + q); 
    //console.log("z"); 
    //console.log(z); 
    ...
    //console.log("reg exp: ");
    //console.log(r);  
    //console.log("Inside split --> with index: " + q); 
...
function splitMatch(s, q, r) {
  if (r instanceof RegExp) {
    ...
    //console.log ("splitMatch -- calling the match"); 
    return r.match(s, q) // this should be on the regexp prototype??
  }
