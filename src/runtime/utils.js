...
function isPrimitive(v) { 
  switch (typeof v) {
    ...
    case 'object'    : if (v === null) { return true } else { return false }
...
function toNumber (x) {
  switch (typeof x) { 
    ...
    case 'boolean'   : if (x) { return 1 } else { return 0 } 
    ...
    default          : throw new Error (`toNumber: ${x}`);    
  }
} 
...
function toInt (x) { 
  ...
  let posInt = sign(x) * Math.floor(Math.abs(x));

  return modulo(posInt, Math.pow(2,32));
}
...
export { ast2str, str2ast, copyFrom, int2str, bool2str, stringMap, chopExtension, boolXor, toInt, modulo, internalToString, isNaN, getGlobalObject };
