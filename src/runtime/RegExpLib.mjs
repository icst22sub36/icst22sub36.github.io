import { multiline, global, ignoreCase, newCtx } from './CompCtx' 
import { captures, endIndex, nCaps, state } from './State'
import { isFailure, makeSuccess } from './Failure'
import { int2str, toInt, internalToString } from './utils'
import { string2matcher, emptyMatcher } from './RegExpRuntime'
...
function match (str, index) { 
  ...
  var state_x = state(str, this.nCaps, index, caps); 
...
function toString () {
  if (this !== undefined) { throw new TypeError ( "RegExp.prototype.toString cannot be used as a constructor") }
...
function RegExp (m, info) { 
  var nCaps = info.nCaps;
  var str = info.pattern; 
  var ctx = info.flags; 
...
Object.defineProperty(RegExp.prototype, 'ignoreCase', 
  { enumerable: false, 
    configurable: true, 
    get: function () { return this._ignoreCase } }); 
