...
import { ast2str, str2ast, bool2str } from "../runtime/utils";
import {
    backSpaceCharSet,
    singletonCharSet,
    emptyCharSet,
    ...
    controlCharSet,
    nonDigitCharSet,
} from "../runtime/CharSet";
import { min, greedy, max, evaluateQuantifier } from "./Quantifier";
import { newCtx, ctx2str, incrParenIndex, incrParenCount, decrParenCount, getParenIndex, getParenCount, getNCaps } from "../runtime/CompCtx";
import { canonicalise } from "../runtime/RegExpRuntime";
...
function genCharacterSetMatcher(ch_set, invert, ctx, loc) {
    ...
    console.log("genCharacterSetMatcher about to explode"); 
    console.log(ch_set); 
    console.log(loc);
    var ret_str = `characterSetMatcher(${ch_set_str}, ${invert_str}, ${ctx2str(ctx)}, {source: "${loc.source}", start: "${loc.start.offset}", end: "${loc.end.offset}"})`;
    return str2ast(ret_str);
}
...
function genGroupMatcher(m, parenIndex) {
    parenIndex = parenIndex || 0;
    const ret_str = `getGroupMatcher(${ast2str(m)}, ${parenIndex})`;
    return str2ast(ret_str);
}

function genBackReferenceMatcher(i, ctx) {
    assert(typeof i === "number");
    const ret_str = `backReferenceMatcher(${i}, ${ctx2str(ctx)})`;
    return str2ast(ret_str);
}
...
function compile(ctx, re) {
    ...
    switch (re.type) {
        ...
        case "Char":
            ...
            var r = genCharacterSetMatcher(c_char_set, false, ctx, re.loc);
            return r;
        ...
        case "CharacterClass":
            /*console.log("About to process a very inconenient char"); 
            console.log(re); */
            var ret_ranges = evaluateClassRanges(re.expressions, ctx);
            return genCharacterSetMatcher(ret_ranges, re.negative === true, ctx, re.loc);
        ...
        case "Group":
            if (re.capturing) {
                ...
                decrParenCount(ctx);
                //var parenIndex = getParenIndex(ctx);
                return genGroupMatcher(m1, cur_paren_index);
            } else {
                var re1 = re.expression;
        ...
        case "Backreference":
            return genBackReferenceMatcher(re.reference, ctx);
